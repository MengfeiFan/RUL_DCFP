...
lambda_U = 1/p_d*p_f;
lambda_L = 0.05/p_d*p_f; % For lambda, a Uniform dist.
% Setting paramters for MCMC
lambda_0 = 0.5/p_d*p_f; % Inital values for lambda
...
func_Post_lambdaFT = @(x,y_shock) Post_lambdaFT(x,y_shock,para); % Handles to posterior dist. of lambda

% simulation
x_post = zeros(n_iter,1); % Allocate memory for MCMC samples
x_prev = [0;0.5;0.5;lambda_0];
...
% load data from Ke's numerical example
load('data.mat'); 
y_true = y;
...
for k = 2:T
   ...
   elseif k == TTF_shock
    ...
    %         disp([num2str(i) '/' num2str(n_iter)])
            ...
            y_shock = [y(:,k),(k-1)*tau]';
            j = 4;
            x_cur = MCMC_MH_iter(x_prev,@Update_lambda,j,func_Post_lambdaFT,@PropDensity_lambda,y_shock,para);
            x_post(i) = x_cur(4); % Record the new sample
       end
       lambda_set = x_post;
       lambda_mean(k) = mean(lambda_set);
       ...
       lambda_mean(k) = lambda_mean(k-1);
       lambda_record(:,k) = lambda_record(:,k-1);
   end
end
lambda_d = lambda_mean*p_d/p_f;
p = zeros(T,1);
for i=1:T
    p(i,1) = 1-exp(-lambda_mean(i)*tau);
end
%% RUL prediction
...
% TTF_shock = ceil(exprnd(1/lambda_0)/tau);
% TTF_shock = 10000;
...
RUL_deg = zeros(Ns,T); % Time to failure
...
for i = 2:T 
    ...
    for j = 1:Ns
        ...
        elseif obs(t,xkm,0) > y_th % if the current time has already failed
            ...
            % Search for the TTF
            lambda_f = lambda_record(j,i);
            T_shock = exprnd(1/lambda_f);
            RUL_shock(j,i) = ceil(T_shock/tau);
            k = t+1;
            while 1
            ...
            RUL(j,i) = min(RUL_deg(j,i),RUL_shock(j,i));
        end
    end
end
...
legend('Estimated x','Observation data','90% belief interval lower','90% belief interval upper','Location','SouthEast');
xlabel('CM point')
ylabel('x')
title('Evolution of degradation')
% Plot RUL VS t
figure
plot(2:T,sum(RUL(:,2:T).*sample_w(:,2:T)),'b-')
hold on
